function minimize!(obj::ObjWrapper, x0,
                   scheme::Union{GradientDescent{<:Direct}, GradientDescent{<:Inverse}, QuasiNewton}, options=OptOptions(),
                   cache=preallocate_qn_caches_inplace(x0))

    minimize!(obj, (x0, nothing), (scheme, Backtracking()), options, cache)
end
function minimize!(obj::ObjWrapper, x0,
                   approach::Tuple{<:QuasiNewton, <:LineSearch}, options=OptOptions(),
                   cache=preallocate_qn_caches_inplace(x0))
...
function minimize!(obj::ObjWrapper, state0::Tuple, approach::Tuple{<:QuasiNewton, <:LineSearch},
    ...
    #========================
         First iteration
    ========================#
    x, fx, ∇fx, z, fz, ∇fz, B, is_converged = iterate!(cache, x, fx, ∇fx, z, fz, ∇fz, B, approach, obj, options)

    iter = 1
    while iter <= options.maxiter && !is_converged
        iter += 1
        # take a step and update approximation
        x, fx, ∇fx, z, fz, ∇fz, B, is_converged = iterate!(cache, x, fx, ∇fx, z, fz, ∇fz, B, approach, obj, options, false)
...
function iterate!(cache, x, fx::Tf, ∇fx, z, fz, ∇fz, B, approach::Tuple{<:Any, <:LineSearch}, obj, options, is_first=nothing) where Tf
