...
const inferredhimmelblau = TwiceDiffed(himmelblau_twicediff, true)
const sl = @SVector([0.0,0.0])
const state0newton = (@SVector([2.0,2.0]), I+sl*sl')
@testset "Newton" begin
    ...
    @testset "Newton linsolve" begin
        res_qr = minimize!(TwiceDiffed(himmelblau!), (copy([2.0,2.0]), [0.0 0.0;0.0 0.0]), LineSearch(Newton(; linsolve=(d, B, g)->ldiv!(d, qr(B), g))), MinOptions())
        @test norm(res_qr.info.∇fz, Inf) < 1e-8
        res_qr = minimize(TwiceDiffed(himmelblau!), (copy([2.0,2.0]), [0.0 0.0;0.0 0.0]), LineSearch(Newton(; linsolve=(B, g)->qr(B)\g)), MinOptions())
        @test norm(res_qr.info.∇fz, Inf) < 1e-8
        res_lu = minimize!(TwiceDiffed(himmelblau!), (copy([2.0,2.0]), [0.0 0.0;0.0 0.0]), LineSearch(Newton(; linsolve=(d, B, g)->ldiv!(d, lu(B), g))), MinOptions())
        @test norm(res_lu.info.∇fz, Inf) < 1e-8
        res_lu = minimize(TwiceDiffed(himmelblau!), (copy([2.0,2.0]), [0.0 0.0;0.0 0.0]), LineSearch(Newton(; linsolve=(B, g)->lu(B)\g)), MinOptions())
        @test norm(res_lu.info.∇fz, Inf) < 1e-8
    end
