...
for _method in (GradientDescent, BFGS, DBFGS, SR1)
# for _method in (GradientDescent, BFGS, DBFGS, DFP, SR1)
    ...
    for m in (Inverse(), Direct())
        ...
        @printf("%2.2e  %2.2e %2.2e  %d\n", norm(res.info.z-xopt,Inf), norm(res.info.∇fz, Inf), res.info.fz, res.info.iter)
        res = minimize!(obj_inplace, copy(x0), LineSearch(_method(m)), MinOptions())
        print("NN! $_method    $mtxt")
        @printf("%2.2e  %2.2e %2.2e  %d\n", norm(res.info.z-xopt,Inf), norm(res.info.∇fz, Inf), res.info.fz, res.info.iter)
...
# for _method in (GradientDescent, BFGS, DBFGS, DFP, SR1)
    ...
    for m in (Inverse(), Direct())
        ...
        print("NN  $_method    $mtxt")
        @printf("%2.2e  %2.2e %2.2e  %d\n", minimum([norm(res.info.z-xopt,Inf) for xopt in minimizers]), norm(res.info.∇fz, Inf), res.info.fz, res.info.iter)
